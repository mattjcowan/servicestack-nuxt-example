<template>
  <div>
    ...
    <p>
      You can use <a
        target="_blank"
        href="https://nuxtjs.org/guide/commands#production-deployment">nuxt generate</a>, to serve this page as a static page.
        It uses <a
          target="_blank"
          href="https://nuxtjs.org/guide/async-data">asyncData</a> to fetch data prior to rendering.
    </p>
    <p>
    ...
    <p>
      ...
      see the <a
        target="_blank"
        href="https://nuxtjs.org/api/configuration-generate">generate property</a> api.
        A good example of this is the <a
          target="_blank"
          href="https://github.com/nuxt/nuxtjs.org/blob/master/nuxt.config.js#L31">nuxtjs.org src on GitHub</a>,
          which builds it's pages at build-time from <a
            target="_blank"
            href="https://github.com/nuxt/docs">an api that serves a GitHub repo via webhooks</a>.
    </p>
    <h2>Server Plugins</h2>
    <ul>
      <li
        v-for="plugin in plugins"
        :key="plugin">{{ plugin }}</li>
    </ul>
    <h2>Server Routes</h2>
    <table
      cellpadding="0"
      ...
      <tbody>
        <tr
          ...
          :key="route.path"><td>{{ route.requestType }}</td><td>{{ route.path }}</td><td>{{ route.verbs.join(', ') }}</td></tr>
      </tbody>
    </table>
